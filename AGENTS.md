...
The Windows adaptations (MinGW CC fix for tree-sitter, Program Files dotnet,
pwsh terminals, Neovide guifont and transparency guards) and the plugin lock
fixes (monokai-pro source correction, stale oil.nvim removal) are upstreamed
in treramey/nvim, so the submodule working tree stays clean. Make Neovim
changes in the nvim repo, then bump the pin here with
`git -C home/dot_config/nvim pull && git add home/dot_config/nvim`.
`run_before_00-init-nvim-submodule` scripts (`.ps1` on Windows, `.sh` on
Linux) initialize the submodule on every apply, so a fresh clone heals itself.
